...
import { useMobileOptimizations } from "src/shared/hooks/useMobileOptimizations";
...
// Pomodoro Analytics Integration
import { QuestAnalyticsService } from "src/features/pomodoro/services/questAnalyticsService";
...
export const AnalyticsTab: React.FC<Props> = ({ plugin }) => {
  ...
  // Mobile optimizations and swipe handling
  ...
  const swipeHandlers = useSwipe(
    () => {
      // Swipe left - next analytics tab
      const currentIndex = analyticsTabsOrder.findIndex(tab => tab === activeTab);
      const nextIndex = (currentIndex + 1) % analyticsTabsOrder.length;
      setActiveTab(analyticsTabsOrder[nextIndex]);
    },
    () => {
      // Swipe right - previous analytics tab
      const currentIndex = analyticsTabsOrder.findIndex(tab => tab === activeTab);
      const prevIndex = currentIndex === 0 ? analyticsTabsOrder.length - 1 : currentIndex - 1;
      setActiveTab(analyticsTabsOrder[prevIndex]);
    }
  );
  ...
  // Pomodoro Analytics Data
  const [questAnalytics] = useState(() => new QuestAnalyticsService());
  const [pomodoroProductivityInsights, setPomodoroProductivityInsights] = useState<any>(null);
  const [questComparisons, setQuestComparisons] = useState<any[]>([]);
  const [pomodoroWeeklyStats, setPomodoroWeeklyStats] = useState<{
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        let insights;
        try {
          insights = questAnalytics.getProductivityInsights();
        } catch (error) {
          console.warn('Failed to get productivity insights, using fallback:', error);
          insights = null;
        }
        setPomodoroProductivityInsights(insights);
        
        // Get quest comparisons with fallback
        let comparisons: any[] = [];
        try {
          comparisons = questAnalytics.getQuestComparisons();
        } catch (error) {
          console.warn('Failed to get quest comparisons, using empty array:', error);
          comparisons = [];
        }
        setQuestComparisons(comparisons);
        
        // Calculate weekly Pomodoro stats with fallback
        let pomodoroStats;
        try {
          pomodoroStats = PomodoroStatsManager.loadStats();
        } catch (error) {
          console.warn('Failed to load pomodoro stats, using fallback:', error);
          pomodoroStats = { thisWeekSessions: 0 };
        }
        
        const weeklyStats = {
          sessionsCompleted: pomodoroStats.thisWeekSessions || 0,
          totalTimeSpent: (pomodoroStats.thisWeekSessions || 0) * 25, // Assume 25 min per session
          averageFocusScore: insights?.overallProductivity?.averageFocusScore || 0,
          questsCompleted: insights?.overallProductivity?.totalQuests || 0,
          hyperfocusSessions: 0 // Placeholder - would need to track this
        };
        setPomodoroWeeklyStats(weeklyStats);
      } catch (error) {
        console.error('Failed to load Pomodoro analytics:', error);
        // Set fallback data to prevent crashes
        setPomodoroProductivityInsights(null);
        setQuestComparisons([]);
        setPomodoroWeeklyStats({
          sessionsCompleted: 0,
          totalTimeSpent: 0,
          averageFocusScore: 0,
          questsCompleted: 0,
          hyperfocusSessions: 0
        });
      }
    })();
  }, [plugin, weekStart, weekEnd, questAnalytics]);
          ...
          <Card>
            <div className={styles.skillsGrid}>
              {skillAnalytics.fastestProgressingSkills.slice(0, 5).map((skill) => (
                <div key={skill.skillName} className={styles.skillCard}>
                  <div className={styles.skillHeader}>
                    <div className={styles.skillName}>{skill.skillName || 'Unknown Skill'}</div>
                    <div className={styles.skillProgress}>
                      Level {skill.currentLevel || 0}/{skill.maxLevel || 10}
                    </div>
                  </div>
                  <div className={styles.progressBar}>
                    <div 
                      className={styles.progressFill} 
                      style={{ width: `${skill.progressToNext || 0}%` }}
                    />
                  </div>
                  <div className={styles.skillStats}>
                    <div className={styles.skillStat}>
                      <span>Progress Rate:</span>
                      <span className={styles.skillStatValue}>+{(skill.progressionRate || 0).toFixed(1)} CP/day</span>
                    </div>
                    <div className={styles.skillStat}>
                      <span>To Next Level:</span>
                      <span className={styles.skillStatValue}>{Math.round(100 - (skill.progressToNext || 0))}%</span>
                    </div>
                  </div>
                </div>
              ))}
  ...
  // Pomodoro Analytics Tab Content
  const renderPomodoroTab = () => {
    // Provide fallback data to prevent crashes
    const weeklyStats = pomodoroWeeklyStats || {
      sessionsCompleted: 0,
      totalTimeSpent: 0,
      averageFocusScore: 0,
      questsCompleted: 0,
    ...
    const insights = pomodoroProductivityInsights || {
      overallProductivity: {
        totalSessions: 0,
        completionRate: 0,
      ...
      questPerformance: {
        easiestQuestType: 'N/A',
        hardestQuestType: 'N/A',
        mostRewardingQuestType: 'N/A',
        averageCompletionTime: 0
      },
      sessionOptimization: {
        bestSessionTypes: ['Classic'],
        optimalSessionLength: 25,
        recommendedBreakFrequency: 4
      },
      timePatterns: {
        mostProductiveHours: [],
        weekendProductivity: 0,
        weekdayProductivity: 0
      },
    ...
    <>
      ...
      <div className={styles.cardsRow}>
        <Card className={styles.card}>
          ...
          <div className={styles.bigNumber}>{weeklyStats.sessionsCompleted}</div>
          <div className={styles.subtle}>completed this week</div>
        </Card>

        <Card className={styles.card}>
          <div className={styles.cardHeader}>Focus Score</div>
          <div className={styles.bigNumber}>
            {Math.round(weeklyStats.averageFocusScore)}%
          </div>
          <div className={styles.subtle}>average focus</div>
        </Card>

        <Card className={styles.card}>
          <div className={styles.cardHeader}>Time Spent</div>
          <div className={styles.bigNumber}>
            {Math.round(weeklyStats.totalTimeSpent / 60)}
          </div>
          <div className={styles.subtle}>hours focused</div>
        </Card>

        <Card className={styles.card}>
          <div className={styles.cardHeader}>Quests</div>
          <div className={styles.bigNumber}>{weeklyStats.questsCompleted}</div>
          <div className={styles.subtle}>completed</div>
        </Card>
      </div>

      {/* Overall Productivity Insights */}
      <div className={styles.sectionHeader}>Overall Productivity Insights</div>
      <div className={styles.cardsRow}>
        <Card className={styles.card}>
          <div className={styles.cardHeader}>Total Sessions</div>
          <div className={styles.bigNumber}>{insights.overallProductivity.totalSessions}</div>
          <div className={styles.subtle}>all time</div>
        </Card>

        <Card className={styles.card}>
          <div className={styles.cardHeader}>Completion Rate</div>
          <div className={styles.bigNumber}>
            {Math.round(insights.overallProductivity.completionRate * 100)}%
          </div>
          <div className={styles.subtle}>quest completion</div>
        </Card>

        <Card className={styles.card}>
          <div className={styles.cardHeader}>Avg Session</div>
          <div className={styles.bigNumber}>
            {Math.round(insights.overallProductivity.averageSessionDuration)}m
          </div>
          <div className={styles.subtle}>session duration</div>
        </Card>

        <Card className={styles.card}>
          <div className={styles.cardHeader}>Total Time</div>
          <div className={styles.bigNumber}>
            {Math.round(insights.overallProductivity.totalTimeSpent / 60)}h
          </div>
          <div className={styles.subtle}>total focused</div>
        </Card>
      </div>

      {/* Quest Performance */}
      <div className={styles.sectionHeader}>Quest Performance</div>
      <Card>
        <div className={styles.questPerformanceGrid}>
          <div className={styles.performanceItem}>
            <div className={styles.performanceLabel}>Easiest Quest Type</div>
            <div className={styles.performanceValue}>
              {insights.questPerformance.easiestQuestType}
            </div>
          </div>
          <div className={styles.performanceItem}>
            <div className={styles.performanceLabel}>Hardest Quest Type</div>
            <div className={styles.performanceValue}>
              {insights.questPerformance.hardestQuestType}
            </div>
          </div>
          <div className={styles.performanceItem}>
            <div className={styles.performanceLabel}>Most Rewarding</div>
            <div className={styles.performanceValue}>
              {insights.questPerformance.mostRewardingQuestType}
            </div>
          </div>
          <div className={styles.performanceItem}>
            <div className={styles.performanceLabel}>Avg Completion</div>
            <div className={styles.performanceValue}>
              {Math.round(insights.questPerformance.averageCompletionTime)}m
            </div>
          </div>
        </div>
      </Card>
      ...
      <div className={styles.sectionHeader}>Session Optimization</div>
      <Card>
        <div className={styles.optimizationGrid}>
          <div className={styles.optimizationItem}>
            <div className={styles.optimizationLabel}>Best Session Types</div>
            <div className={styles.optimizationValue}>
              {insights.sessionOptimization.bestSessionTypes.join(', ')}
            </div>
          </div>
          <div className={styles.optimizationItem}>
            <div className={styles.optimizationLabel}>Optimal Length</div>
            <div className={styles.optimizationValue}>
              {insights.sessionOptimization.optimalSessionLength}m
            </div>
          </div>
          <div className={styles.optimizationItem}>
            <div className={styles.optimizationLabel}>Break Frequency</div>
            <div className={styles.optimizationValue}>
              Every {insights.sessionOptimization.recommendedBreakFrequency} sessions
            </div>
          </div>
        </div>
      </Card>
      ...
      {/* Time Patterns */}
      <div className={styles.sectionHeader}>Time Patterns</div>
      <Card>
        <div className={styles.timePatternsGrid}>
          <div className={styles.timePatternItem}>
            <div className={styles.timePatternLabel}>Most Productive Hours</div>
            <div className={styles.timePatternValue}>
              {insights.timePatterns.mostProductiveHours.length > 0 
                ? insights.timePatterns.mostProductiveHours.join(', ')
                : 'Not enough data'
              }
            </div>
          </div>
          <div className={styles.timePatternItem}>
            <div className={styles.timePatternLabel}>Weekend vs Weekday</div>
            <div className={styles.timePatternValue}>
              Weekend: {Math.round(insights.timePatterns.weekendProductivity)}% | 
              Weekday: {Math.round(insights.timePatterns.weekdayProductivity)}%
            </div>
          </div>
        </div>
      </Card>
      ...
      {insights.improvementSuggestions.length > 0 && (
        <>
          <div className={styles.sectionHeader}>Improvement Suggestions</div>
          <Card>
            <div className={styles.suggestionsList}>
              {insights.improvementSuggestions.map((suggestion: string, index: number) => (
      ...
      {/* Quest Comparisons */}
      {questComparisons.length > 0 && (
        <>
          <div className={styles.sectionHeader}>Quest Performance Comparison</div>
          <Card>
            <div className={styles.questComparisonGrid}>
              {questComparisons.slice(0, 4).map((comparison) => (
                <div key={comparison.questId || 'unknown'} className={styles.comparisonCard}>
                  <div className={styles.comparisonTitle}>{comparison.questTitle || 'Unknown Quest'}</div>
                  <div className={styles.comparisonStats}>
                    <div className={styles.comparisonStat}>
                      <span>Focus Score:</span>
                      <span className={styles.comparisonValue}>{Math.round(comparison.averageFocusScore || 0)}%</span>
                    </div>
                    <div className={styles.comparisonStat}>
                      <span>Avg Time:</span>
                      <span className={styles.comparisonValue}>{Math.round(comparison.averageCompletionTime || 0)}m</span>
                    </div>
                    <div className={styles.comparisonStat}>
                      <span>XP/Min:</span>
                      <span className={styles.comparisonValue}>{(comparison.xpPerMinute || 0).toFixed(1)}</span>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {/* Achievements */}
      <div className={styles.sectionHeader}>Pomodoro Achievements</div>
      <div className={styles.cardsRow}>
        <Card className={styles.card}>
          <div className={styles.cardHeader}>Streaks</div>
          <div className={styles.bigNumber}>{insights.achievements.streaks}</div>
          <div className={styles.subtle}>consecutive days</div>
        </Card>

        <Card className={styles.card}>
          <div className={styles.cardHeader}>Perfect Sessions</div>
          <div className={styles.bigNumber}>{insights.achievements.milestones}</div>
          <div className={styles.subtle}>milestones</div>
        </Card>
        ...
        <Card className={styles.card}>
          <div className={styles.cardHeader}>Focus Mastery</div>
          <div className={styles.bigNumber}>{insights.achievements.milestones}</div>
          <div className={styles.subtle}>high focus sessions</div>
        </Card>
      </div>

      {/* Empty State */}
      {insights.overallProductivity.totalSessions === 0 && (
        <div className={styles.emptyState}>
          <div className={styles.emptyIcon}>🍅</div>
          <div className={styles.emptyTitle}>No Pomodoro Data Yet</div>
          <div className={styles.emptyDescription}>
            Start using Pomodoro sessions with quests to see your productivity analytics here!
          </div>
        </div>
      )}
    </>
  ...
  return (
    <div className={styles.root} {...swipeHandlers}>
      ...
      <div className={styles.tabNavigation}>
        ...
        <button 
          className={`${styles.tabButton} ${activeTab === 'pomodoro' ? styles.activeTab : ''}`}
