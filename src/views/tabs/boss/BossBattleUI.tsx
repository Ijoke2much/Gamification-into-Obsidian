...
export const BossBattleUI: React.FC<BossBattleUIProps> = ({ plugin }) => {
    ...
    const [playerBattleStats, setPlayerBattleStats] = useState<PlayerStats>({ strength: 1, endurance: 1, focus: 1, intelligence: 1, creativity: 1, motivation: 1, patience: 1, agility: 1, charisma: 1 });
    ...
    const createDemoBoss = (): BossData => {
        ...
        const demoQuest: Quest = {
            ...
            stats: ['focus', 'intelligence', 'creativity', 'strength'],
            xp: 0,
            cp: 0,
            coins: 0,
    ...
    const handleAbilityUse = async (abilityName: string, playerStat: number) => {
        ...
        if (skillBasedMove && skillBasedBoss && isSkillDemoBoss) {
            ...
            } else if (abilityName === 'Deadline Rush' && playerData?.stats?.agility) {
                statBonus = Math.floor(playerData.stats.agility / 3);
            ...
            return (
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                            <div style={{ fontWeight: 'bold', fontSize: '14px', color: '#ffd700' }}>
                                {selectedBoss.boss.name}
                                {isDemoBoss && <span style={{ color: '#10b981', marginLeft: '5px' }}>(DEMO)</span>}
                            </div>
                            <div style={{ 
                                width: '120px', 
                                height: '8px',
        ...
        return (
            <div style={{
                ...
                <div style={{
                    ...
                    <h1 style={{ 
                        ...
                        🎮 Boss Battle: {selectedBoss.boss.name}
                            {isDemoBoss && <span style={{ color: '#10b981', marginLeft: '10px' }}>(DEMO)</span>}
                    </h1>
                ...
                {/* Left Panel - Player Stats & Equipment */}
                    ...
                    <h3 style={{ color: '#60a5fa', marginBottom: '16px', textAlign: 'center' }}>
                        🛡️ Player Stats
                    </h3>
                    ...
                    <div style={{ marginBottom: '20px' }}>
                        {Object.entries(playerBattleStats).map(([stat, value]) => (
                            <div key={stat} style={{
                                ...
                                <span style={{ textTransform: 'capitalize', color: '#94a3af' }}>
                                    {stat}:
                                </span>
                        ...
                        {selectedBoss.boss.id === 'skill-demo-boss-001' && availableMoves.length > 0 ? 
                            availableMoves.slice(0, 4).map(move => {
                                ...
                                [
                                    ...
                                    { name: 'Deadline Rush', icon: '⚡', damage: 70, color: '#84cc16', stat: 'agility', statIcon: '🏃' }
                                ].map((move, index) => (
                                    <button
                                        key={move.name}
    ...
    return (
        <div style={{
            ...
            <div style={{ minHeight: 'calc(100vh - 100px)' }}>
                ...
                {activeTab === 'creation' && renderBossCreationTab()}
