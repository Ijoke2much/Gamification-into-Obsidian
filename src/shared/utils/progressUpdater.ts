...
// --- Test function to verify stat finding ---
export async function testStatFinding(vault: Vault, statName: string) {
  console.log(`[testStatFinding] Testing stat finding for: ${statName}`);
  const statPath = await findStatPath(vault, statName);
  console.log(`[testStatFinding] Found stat path: ${statPath}`);
  return statPath;
}

// --- Test function to list all stats in Master-Class/Stats directory ---
export async function listAllStats(vault: Vault) {
  console.log(`[listAllStats] Listing all stats in Master-Class/Stats directory`);
  const allFiles = vault.getAllLoadedFiles();
  const statFiles = allFiles.filter(file =>
    file instanceof TFile && file.path.startsWith('SkillTree/Master-Class/Stats/')
  );

  console.log(`[listAllStats] Found ${statFiles.length} stat files:`);
  for (const file of statFiles) {
    if (file instanceof TFile) {
      try {
        const content = await vault.read(file);
        const { data } = matter(content);
        console.log(`[listAllStats] File: ${file.path}, Name: ${data.name || data.title || 'NO NAME'}, Level: ${data.level || 'NO LEVEL'}`);
      } catch (err) {
        console.log(`[listAllStats] Error reading ${file.path}:`, err);
      }
    }
  }
}

// --- Test function to verify skill finding and metadata ---
export async function testSkillFinding(vault: Vault, skillName: string) {
  console.log(`[testSkillFinding] Testing skill finding for: ${skillName}`);
  const skillPath = await findSkillPath(vault, skillName);
  console.log(`[testSkillFinding] Found skill path: ${skillPath}`);

  if (skillPath) {
    const { getSkillMetadata } = await import('./skillDiscovery');
    const skillMetadata = await getSkillMetadata(vault, skillName);
    console.log(`[testSkillFinding] Skill metadata:`, skillMetadata);

    if (skillMetadata) {
      console.log(`[testSkillFinding] Class: ${skillMetadata.class}`);
      console.log(`[testSkillFinding] Class Path: ${skillMetadata.classPath}`);
      console.log(`[testSkillFinding] Master Class: ${skillMetadata.masterClass}`);
      console.log(`[testSkillFinding] Master Class Path: ${skillMetadata.masterClassPath}`);

      // Check if class file exists
      const classFile = vault.getAbstractFileByPath(skillMetadata.classPath);
      console.log(`[testSkillFinding] Class file exists: ${!!classFile}`);

      // Check if master class file exists
      const masterClassFile = vault.getAbstractFileByPath(skillMetadata.masterClassPath);
      console.log(`[testSkillFinding] Master class file exists: ${!!masterClassFile}`);
    }
  }

  return { skillPath, skillMetadata: skillPath ? await import('./skillDiscovery').then(m => m.getSkillMetadata(vault, skillName)) : null };
}

// --- Comprehensive debugging function for quest completion issues ---
export async function debugQuestCompletion(vault: Vault, questData: { skills?: string[], stats?: string[], cp: number }) {
  console.log(`[debugQuestCompletion] Starting comprehensive debug for quest completion`);
  console.log(`[debugQuestCompletion] Quest data:`, questData);

  const skills = questData.skills || [];
  const stats = questData.stats || [];

  console.log(`[debugQuestCompletion] === DEBUGGING SKILLS ===`);
  for (const skillName of skills) {
    console.log(`[debugQuestCompletion] --- Testing skill: ${skillName} ---`);
    await testSkillFinding(vault, skillName);
  }

  console.log(`[debugQuestCompletion] === DEBUGGING STATS ===`);
  for (const statName of stats) {
    console.log(`[debugQuestCompletion] --- Testing stat: ${statName} ---`);
    await testStatFinding(vault, statName);
  }

  console.log(`[debugQuestCompletion] === LISTING ALL SKILL FILES ===`);
  const allFiles = vault.getAllLoadedFiles();
  ...
  console.log(`[debugQuestCompletion] Found ${skillFiles.length} skill files:`);
  for (const file of skillFiles) {
    console.log(`[debugQuestCompletion] - ${file.path}`);
  }

  console.log(`[debugQuestCompletion] === LISTING ALL STAT FILES ===`);
  const statFiles = allFiles.filter(f =>
    f instanceof TFile &&
    (f.path.includes('/Stats/') || f.path.includes('/stats/')) &&
    f.path.endsWith('.md')
  );
  console.log(`[debugQuestCompletion] Found ${statFiles.length} stat files:`);
  for (const file of statFiles) {
    console.log(`[debugQuestCompletion] - ${file.path}`);
  }

  console.log(`[debugQuestCompletion] === LISTING ALL CLASS FILES ===`);
  ...
  console.log(`[debugQuestCompletion] Found ${classFiles.length} class files:`);
  for (const file of classFiles) {
    console.log(`[debugQuestCompletion] - ${file.path}`);
  }

  console.log(`[debugQuestCompletion] Debug complete!`);
}
...
export async function updatePlayerData(vault: Vault, xp: number, coins: number, cp: number = 0) {
  ...
  try {
    console.log(`[updatePlayerData] Called with xp=${xp}, coins=${coins}, cp=${cp}`);
    ...
    console.log(`[updatePlayerData] Starting level-up check. Current level: ${oldLevel}, XP: ${frontmatter.xp}, XP Required: ${frontmatter.xpRequired}`);
    ...
    if (!frontmatter.xpRequired || typeof frontmatter.xpRequired !== 'number') {
      ...
      console.log(`[updatePlayerData] Fixed xpRequired to ${frontmatter.xpRequired} for level ${oldLevel}`);
    }
    ...
    while ((typeof frontmatter.xp === 'number' ? frontmatter.xp : Number(frontmatter.xp) || 0) >= (typeof frontmatter.xpRequired === 'number' ? frontmatter.xpRequired : getXpRequired(oldLevel)) && levelUps < maxLevelUps) {
      ...
      console.log(`[updatePlayerData] Level up! XP: ${currentXP} >= ${currentXPRequired}`);
      ...
      console.log(`[updatePlayerData] New level: ${frontmatter.level}, XP: ${frontmatter.xp}, XP Required: ${frontmatter.xpRequired}`);
    ...
    if (levelUps >= maxLevelUps) {
      console.warn(`[updatePlayerData] Reached maximum level-ups (${maxLevelUps}), stopping to prevent infinite loop`);
    }
    ...
    if (!frontmatter.xpRequired || typeof frontmatter.xpRequired !== 'number') {
      ...
      console.log(`[updatePlayerData] Final xpRequired fix: ${frontmatter.xpRequired} for level ${frontmatter.level}`);
    }
...
async function updateAssociatedStats(vault: Vault, skillPath: string, skillName: string, cp: number) {
  try {
    console.log(`[updateAssociatedStats] Finding stats for skill: ${skillName}`);
    ...
    if (!skillMetadata || !skillMetadata.stats) {
      console.log(`[updateAssociatedStats] No stats found for skill: ${skillName}`);
      return;
    }

    console.log(`[updateAssociatedStats] Found ${Object.keys(skillMetadata.stats).length} associated stats:`, Object.keys(skillMetadata.stats));

    // Each stat receives the same CP as the skill (not divided)
    const statCP = Math.floor(cp * 0.5); // 50% of skill CP to each stat

    if (statCP <= 0) {
      console.log(`[updateAssociatedStats] Calculated stat CP is ${statCP}, skipping stat updates`);
      return;
    }

    console.log(`[updateAssociatedStats] Distributing ${statCP} CP to each of ${Object.keys(skillMetadata.stats).length} stats (${statCP} CP per stat)`);

    // Update each associated stat
    for (const [statName, statPath] of Object.entries(skillMetadata.stats)) {
      try {
        console.log(`[updateAssociatedStats] Updating stat: ${statName} at ${statPath}`);
        const statLeveledUp = await updateStatProgress(vault, statPath, statCP);
        if (statLeveledUp) {
          console.log(`[updateAssociatedStats] Stat ${statName} leveled up!`);
        }
      } catch (err) {
...
async function findStatPath(vault: Vault, statName: string): Promise<string | null> {
  try {
    console.log(`[findStatPath] Looking for stat: ${statName}`);
    const allFiles = vault.getAllLoadedFiles();
    console.log(`[findStatPath] Total files in vault: ${allFiles.length}`);

    // First, let's see what stat files exist in the Master-Class/Stats directory
    const statFiles = allFiles.filter(file =>
      file instanceof TFile && file.path.startsWith('SkillTree/Master-Class/Stats/')
    );
    console.log(`[findStatPath] Found ${statFiles.length} files in SkillTree/Master-Class/Stats/:`,
      statFiles.map(f => f.path));
    ...
    for (const file of allFiles) {
      if (file instanceof TFile) {
        ...
        if (
          ...
          console.log(`[findStatPath] Checking file: ${file.path}`);
          try {
            const content = await vault.read(file);
            const { data } = matter(content);
            console.log(`[findStatPath] File ${file.path} has data:`, data);

            // Check both name and title fields for stat identification
            const statNameInFile = data.name || data.title || '';
            console.log(`[findStatPath] Comparing "${statName}" with "${statNameInFile}"`);

            if (statNameInFile === statName) {
              console.log(`[findStatPath] Found stat ${statName} at path: ${file.path}`);
              return file.path;
            }
          } catch (readError) {
            console.log(`[findStatPath] Error reading file ${file.path}:`, readError);
          }
        }
      }
    }
    console.log(`[findStatPath] Stat ${statName} not found in any files`);
    return null;
  } catch (err) {
...
export async function distributeCPFromQuest(vault: Vault, quest: { skills?: string[], stats?: string[], cp: number }) {
  try {
    console.log(`[distributeCPFromQuest] === STARTING QUEST CP DISTRIBUTION ===`);
    console.log(`[distributeCPFromQuest] Raw quest object:`, JSON.stringify(quest, null, 2));
    console.log(`[distributeCPFromQuest] Distributing ${quest.cp} CP from quest`);
    console.log(`[distributeCPFromQuest] Quest data:`, quest);

    const skills = quest.skills || [];
    const stats = quest.stats || [];
    const fullCP = quest.cp; // Full CP amount for each entity

    console.log(`[distributeCPFromQuest] Extracted skills:`, skills);
    console.log(`[distributeCPFromQuest] Extracted stats:`, stats);
    console.log(`[distributeCPFromQuest] Extracted CP:`, fullCP);

    console.log(`[distributeCPFromQuest] Skills to update:`, skills);
    console.log(`[distributeCPFromQuest] Stats to update:`, stats);
    console.log(`[distributeCPFromQuest] CP per entity:`, fullCP);

    // Run comprehensive debug if there are skills or stats to process
    if (skills.length > 0 || stats.length > 0) {
      console.log(`[distributeCPFromQuest] Running comprehensive debug...`);
      await debugQuestCompletion(vault, quest);
    }
    ...
    for (const skillNameRaw of skills) {
      ...
      console.log(`[distributeCPFromQuest] Processing skill: ${skillName}`);
      if (fullCP > 0) {
        ...
        console.log(`[distributeCPFromQuest] Skill path for ${skillName}: ${skillPath}`);
        new Notice(`[CP Debug] Skill: ${skillName} → ${skillPath || 'NOT FOUND'}`);

        if (skillPath) {
          const leveledUp = await updateSkillProgress(vault, skillPath, fullCP);
          new Notice(`[CP Debug] Applied ${fullCP} CP to skill ${skillName}. Leveled: ${leveledUp}`);
          if (leveledUp) {
          ...
          const { getSkillMetadata } = await import('./skillDiscovery');
          const skillMetadata = await getSkillMetadata(vault, skillName);
          console.log(`[distributeCPFromQuest] Skill metadata for ${skillName}:`, skillMetadata);

          if (skillMetadata) {
            console.log(`[distributeCPFromQuest] Found class: ${skillMetadata.class} at path: ${skillMetadata.classPath}`);
            console.log(`[distributeCPFromQuest] Found master class: ${skillMetadata.masterClass} at path: ${skillMetadata.masterClassPath}`);

            // Check if class file exists before updating
            const classFile = vault.getAbstractFileByPath(skillMetadata.classPath);
            if (classFile) {
              console.log(`[distributeCPFromQuest] Updating class ${skillMetadata.class} with ${fullCP} CP`);
              const classLeveledUp = await updateClassProgress(vault, skillMetadata.classPath, fullCP);
              new Notice(`[CP Debug] Applied ${fullCP} CP to class ${skillMetadata.class}. Leveled: ${classLeveledUp}`);
              if (classLeveledUp) {
                levelUps.push(`${skillMetadata.class} (Class)`);
              }
            } else {
              console.log(`[distributeCPFromQuest] WARNING: Class file not found at ${skillMetadata.classPath}`);
              new Notice(`[CP Debug] Class path not found: ${skillMetadata.classPath}`);
            }
            ...
            if (masterClassFile) {
              console.log(`[distributeCPFromQuest] Updating master class ${skillMetadata.masterClass} with ${fullCP} CP`);
              const masterLeveledUp = await updateMasterClassProgress(vault, skillMetadata.masterClassPath, fullCP);
              new Notice(`[CP Debug] Applied ${fullCP} CP to master ${skillMetadata.masterClass}. Leveled: ${masterLeveledUp}`);
              if (masterLeveledUp) {
                levelUps.push(`${skillMetadata.masterClass} (Master Class)`);
              }
            } else {
              console.log(`[distributeCPFromQuest] WARNING: Master class file not found at ${skillMetadata.masterClassPath}`);
              new Notice(`[CP Debug] Master class path not found: ${skillMetadata.masterClassPath || 'N/A'}`);
            }
          } else {
            console.log(`[distributeCPFromQuest] WARNING: No skill metadata found for ${skillName}`);
            new Notice(`[CP Debug] No metadata found for ${skillName}`);
          }
        } else {
          console.log(`[distributeCPFromQuest] WARNING: Could not find skill file for ${skillName}`);
          new Notice(`[CP Debug] Could not locate skill file for ${skillName}`);
        }
      }
    }
    ...
    console.log(`[distributeCPFromQuest] Starting manual stat distribution...`);
    console.log(`[distributeCPFromQuest] Stats array length: ${stats.length}`);
    console.log(`[distributeCPFromQuest] Stats array contents:`, stats);
    console.log(`[distributeCPFromQuest] fullCP value:`, fullCP);

    for (const statName of stats) {
      console.log(`[distributeCPFromQuest] Processing stat: ${statName}`);
      if (fullCP > 0) {
        const statPath = await findStatPath(vault, statName);
        console.log(`[distributeCPFromQuest] Stat path for ${statName}:`, statPath);
        if (statPath) {
          console.log(`[distributeCPFromQuest] Updating stat progress for ${statName} with ${fullCP} CP`);
          const leveledUp = await updateStatProgress(vault, statPath, fullCP);
          console.log(`[distributeCPFromQuest] Stat ${statName} leveled up:`, leveledUp);
          if (leveledUp) {
            levelUps.push(`${statName} (Stat)`);
          }
        } else {
          console.log(`[distributeCPFromQuest] WARNING: Could not find stat file for ${statName}`);
        }
      }
    }

    // Optionally, you can show a summary notification for all level-ups
...
export async function checkAndFixPlayerLevel(vault: Vault, playerStoreInstance?: PlayerStoreInterface): Promise<{ level: number; xp: number; xpRequired: number; leveledUp: boolean }> {
  ...
  try {
    console.log("[checkAndFixPlayerLevel] Starting manual level check...");
    ...
    console.log(`[checkAndFixPlayerLevel] Current state - Level: ${currentLevel}, XP: ${currentXP}, XP Required: ${currentXPRequired}`);

    // Ensure xpRequired is correct for current level
    if (currentXPRequired !== getXpRequired(currentLevel)) {
      console.log(`[checkAndFixPlayerLevel] Fixing xpRequired from ${currentXPRequired} to ${getXpRequired(currentLevel)}`);
    ...
    while (currentXP >= currentXPRequired && levelUps < maxLevelUps) {
      console.log(`[checkAndFixPlayerLevel] Level up! XP: ${currentXP} >= ${currentXPRequired}`);

      currentXP -= currentXPRequired;
      currentLevel++;
      currentXPRequired = getXpRequired(currentLevel);

      console.log(`[checkAndFixPlayerLevel] New level: ${currentLevel}, XP: ${currentXP}, XP Required: ${currentXPRequired}`);
    ...
    if (leveledUp) {
      ...
      console.log(`[checkAndFixPlayerLevel] Updated player data - Level: ${currentLevel}, XP: ${currentXP}, XP Required: ${currentXPRequired}`);

      // The player store will automatically dispatch the event, so no need to do it here

      new Notice(`🎉 Level up! You are now level ${currentLevel}!`, 3000);
    } else {
      console.log(`[checkAndFixPlayerLevel] No level ups needed. Current state is correct.`);
    }
