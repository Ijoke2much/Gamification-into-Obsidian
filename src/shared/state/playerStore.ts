...
class PlayerStore {
  ...
  private async initializeStore() {
    ...
    try {
      ...
      // Wait for vault to be fully loaded, retry up to 5 times
      let retries = 0;
      let data: PlayerData | null = null;

      while (retries < 5 && !data) {
        if (retries > 0) {
          console.log(`[PlayerStore] Retry ${retries}/5 - waiting for vault to load files...`);
  ...
  async update(updater: (data: PlayerData) => PlayerData | Promise<PlayerData>): Promise<void> {
    ...
    this.writeQueue.push(async () => {
      try {
        ...
        console.error('Failed to write player data:', message);
      }
    });
