...
export class EnergyAnalyticsService {
    ...
    private async getCurrentEnergyState(): Promise<EnergyState> {
        ...
        try {
            ...
            // Fallback to basic state - GRACEFUL DEGRADATION
            const playerData = await playerStore.get();
            const now = new Date();
            
            if (!playerData) {
                console.warn('[EnergyAnalyticsService] Player data not available, returning default state');
                return {
                    energy: 70,
                    focus: 70,
                    motivation: 70,
                    ...
                    lastRestTime: now,
                    nextRecommendedRest: new Date(now.getTime() + 4 * 60 * 60 * 1000),
                    energyTrend: 'stable'
                };
            }
