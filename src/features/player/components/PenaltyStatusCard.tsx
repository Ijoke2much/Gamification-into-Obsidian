...
export interface PenaltyStatusCardProps {
    /** Mobile: start collapsed — summary strip until expand; defer analytics/coaching load */
    collapsed?: boolean;
}

export const PenaltyStatusCard: React.FC<PenaltyStatusCardProps> = ({ collapsed = false }) => {
    ...
    const [heavyLoaded, setHeavyLoaded] = useState(!collapsed);
    const [sheetOpen, setSheetOpen] = useState(false);

    const loadPlayerOnly = useCallback(async () => {
        try {
            const player = await playerStore.get();
            setPlayerData(player);
        } catch (error) {
            console.error('Error loading player for penalty card:', error);
        }
    }, []);
    ...
    const loadHeavy = useCallback(async () => {
        ...
        try {
            ...
            const [insightsResult, forgivenessResult, coachingResult] = await Promise.allSettled([
            ...
            if (forgivenessResult.status === 'fulfilled') setForgivenessEvents(forgivenessResult.value);
            if (coachingResult.status === 'fulfilled') setCoachingSession(coachingResult.value);
            setHeavyLoaded(true);
        } catch (error) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = playerStore.onChange((change) => {
            if (change.type === 'data-updated') {
                ...
                // Desktop: refresh insights when player changes (same as before)
                if (!collapsed) {
                    void loadHeavy();
                }
            }
        ...
        return unsubscribe;
        // eslint-disable-next-line react-hooks/exhaustive-deps -- mount + collapsed mode only
    }, [collapsed]);

    useEffect(() => {
        if (collapsed && expanded && !heavyLoaded) {
            void loadHeavy();
        }
    }, [collapsed, expanded, heavyLoaded, loadHeavy]);
    ...
    const handleGenerateCoachingSession = async () => {
        ...
        try {
            ...
            if (collapsed) setSheetOpen(true);
        } catch (error) {
    ...
    const activeDebuffs = (playerData.debuffs || []).filter(
        (debuff: Debuff) => !debuff.expiresAt || new Date(debuff.expiresAt) > new Date()
    );

    const hasAnyPenalties =
        debtXP > 0 || debtCoins > 0 || reputation < 0 || activeDebuffs.length > 0;
    const activeBuffs = (playerData.buffs || []).filter(
        (buff: Buff) => !buff.expiresAt || new Date(buff.expiresAt) > new Date()
    );
    ...
    if (debtXP > 0) summaryBits.push(`${debtXP} XP debt`);
    if (debtCoins > 0) summaryBits.push(`${debtCoins} coin debt`);
    if (activeDebuffs.length > 0) summaryBits.push(`${activeDebuffs.length} debuff${activeDebuffs.length === 1 ? '' : 's'}`);
    if (summaryBits.length === 0) summaryBits.push('Clean');

    const coachingCount = coachingSession?.suggestions?.length ?? 0;
    const showBody = !collapsed || expanded;

    const renderCoachingBody = () =>
        coachingSession ? (
            ...
            <p className={styles.noData}>No coaching session yet. Tap Get Coaching.</p>
        );

    return (
        <div
            className={`${styles.penaltyCard}${collapsed ? ` ${styles.penaltyCardMobile}` : ''}`}
            data-penalty-lite={collapsed ? 'true' : 'false'}
        >
            <div className={styles.header}>
                {collapsed ? (
                    ...
                    >
                        <h3>
                            {expanded ? '▾' : '▸'} Recovery
                            <span className={styles.mobileSummary}> · {summaryBits.join(' · ')}</span>
                            {coachingCount > 0 ? (
                                <span className={styles.mobileBadge}> {coachingCount} tip{coachingCount === 1 ? '' : 's'}</span>
                            ) : null}
                            {forgivenessEvents.length > 0 ? (
                                <span className={styles.mobileBadgeForgive}> {forgivenessEvents.length} forgive</span>
                            ) : null}
                        </h3>
                    </button>
                ) : (
                    <>
                        ...
                        <div className={styles.statusIndicators}>
                            ...
                            {insights.length > 0 && (
                                <span className={styles.insight}>{insights.length} Insights</span>
                            )}
                            {forgivenessEvents.length > 0 && (
                                <span className={styles.forgiveness}>
                                    {forgivenessEvents.length} Opportunities
                                </span>
                            )}
                        </div>
                    </>
            ...
            {showBody && (
                <>
                    <div className={styles.quickStatus}>
                        <div className={styles.statusItem}>
                            ...
                            <span className={styles.value}>
                                ...
                                {debtCoins > 0 && (
                                    <span className={styles.debt}>📉 {debtCoins} Coins</span>
                                )}
                                {debtXP === 0 && debtCoins === 0 && (
                        ...
                        <div className={styles.analyticsPanel}>
                            ...
                            {insights.length > 0 ? (
                                ...
                                <p className={styles.noData}>
                                    No insights available. Keep up the good work!
                                </p>
                            )}
                        </div>
                    )}

                    {/* Desktop inline coaching */}
                    {!collapsed && showCoaching && coachingSession && renderCoachingBody()}
                    ...
                    {forgivenessEvents.length > 0 && (
                        <div className={styles.forgivenessPanel}>
                            ...
                            <div className={styles.forgivenessList}>
                                {forgivenessEvents.slice(0, collapsed ? 2 : 3).map((event, index) => (
                                    <div key={index} className={styles.forgivenessItem}>
                                        <h5>{event.name}</h5>
                                        <p>{event.description}</p>
                                        <div className={styles.forgivenessRewards}>
                                            <span>
                                                💰 -{event.rewards.debtReduction.xp} XP, -
                                                {event.rewards.debtReduction.coins} Coins
                                            </span>
                                            <span>
                                                ⭐ +{event.rewards.reputationBoost} Reputation
                                            </span>
                                        </div>
                                    </div>
                                ))}
                    ...
                    {/* Desktop-only buff/debuff lists (mobile has ActiveBuffsCard) */}
                    {!collapsed && activeBuffs.length > 0 && (
                                ...
                                {activeBuffs.map((buff, index) => (
                                    <div key={index} className={styles.buffItem}>
                                        ...
                                        <div className={styles.buffValue}>
                                            ...
                                            {buff.type === 'flat' && (
                                                <span className={styles.valueBadge}>+{buff.value}</span>
                                            )}
                                        </div>
                                    </div>
                                ))}
                    ...
                    {!collapsed && activeDebuffs.length > 0 && (
                                    ...
                                    <div key={index} className={styles.debuffItem}>
                                        <div className={styles.debuffHeader}>
                                            ...
                                            <span className={styles.debuffTime}>
                                                {debuff.expiresAt
                                                    ? formatTimeRemaining(debuff.expiresAt)
                                                    : 'Permanent'}
                                            </span>
                                        </div>
            ...
            {/* Mobile coaching — portal full-screen so it isn't clipped by Player tab overflow */}
            {collapsed &&
                sheetOpen &&
                createPortal(
                    <div
