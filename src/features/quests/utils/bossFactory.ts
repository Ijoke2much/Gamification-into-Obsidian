...
export const BOSS_TEMPLATES: Record<string, unknown> = {
    'single-session': {
        id: 'single-session',
...
export class BossFactory {
    ...
    static createBoss(options: BossCreationOptions): Boss {
        ...
        const baseMoves = this.generateBossMoves(bossType, theme);
        const basePhases = this.generateBossPhases(bossType, theme);
        const rewards = this.generateBossRewards(options, bossType);
        const category = this.determineBossCategory(options);
        const baseVisuals = this.generateBossVisuals(theme, category);
        const baseDialogue = this.generateBossDialogue(theme);
        ...
        return {
            ...
            image: (options.visuals?.avatar ?? theme.defaultAvatar),
            category,
            type: bossType,
            visuals: { ...baseVisuals, ...options.visuals },
            dialogue: { ...baseDialogue, ...options.dialogue },
            timer: options.enableTimer ? { ...this.generateBossTimer(options), ...options.timerConfig } : undefined,
            stats,
            moves: options.moves?.length ? options.moves : baseMoves,
            ...
            rewards,
            phases: options.phases?.length ? options.phases : basePhases,
            currentPhase: 0,
            isDefeated: false,
            createdAt: new Date(),
            difficulty: options.difficulty,
            theme: theme.id,
            weaknesses: options.weaknesses ?? this.generateWeaknesses(theme),
            resistances: options.resistances ?? this.generateResistances(theme),
            specialAbilities: options.specialAbilities ?? this.generateSpecialAbilities(theme),
    ...
    private static selectBossTheme(options: BossCreationOptions): BossTheme {
        if (options.bossTheme && BOSS_THEMES[options.bossTheme]) return BOSS_THEMES[options.bossTheme];
        if (options.associatedSkill) return this.mapSkillToTheme(options.associatedSkill);
        return BOSS_THEMES.procrastination;
    }

    private static mapSkillToTheme(skill: string): BossTheme {
        const s = (skill || '').toLowerCase();
        if (['focus', 'mindfulness'].some(k => s.includes(k))) return BOSS_THEMES.distraction;
        if (['willpower', 'motivation'].some(k => s.includes(k))) return BOSS_THEMES.deadline;
        if (['intelligence', 'analysis', 'logic'].some(k => s.includes(k))) return BOSS_THEMES.complexity;
        if (['creativity', 'design', 'art'].some(k => s.includes(k))) return BOSS_THEMES.perfectionism;
        if (['strength', 'endurance', 'fitness', 'training'].some(k => s.includes(k))) return BOSS_THEMES.overwhelm;
        if (['finance', 'money', 'budget'].some(k => s.includes(k))) return BOSS_THEMES.mortgage;
        return BOSS_THEMES.procrastination;
    }

    private static selectBossName(theme: BossTheme): string {
        return theme.name;
    }

    private static generateBossTitle(theme: BossTheme, questTitle: string): string {
    ...
    private static generateBossMoves(bossType: Boss['type'], theme: BossTheme): BossMove[] {
        const config = this.BOSS_TYPES[bossType];
    ...
    private static generateBossPhases(bossType: Boss['type'], theme: BossTheme): BossPhase[] {
        ...
        for (let i = 0; i < phaseCount; i++) {
            ...
            phases.push({
                ...
                hpThreshold,
                moves: this.getPhaseMoves(bossType, phaseNumber, theme).map(move => move.name), // Convert to string array
    ...
    private static getPhaseMoves(bossType: Boss['type'], phaseNumber: number, theme: BossTheme): BossMove[] {
        const config = this.BOSS_TYPES[bossType];
        const moves: BossMove[] = [];
        ...
        // Get theme-specific moves using the selected theme for consistency
        const themeToUse = theme;
        themeToUse.defaultMoves.slice(0, moveCount - 1).forEach((moveName: string, index: number) => {
            const isStatusMove = index % 2 === 0;
            moves.push({
                name: moveName,
                type: isStatusMove ? 'status' : 'attack',
...
// (mapping now implemented as a private static method on BossFactory)
...
export function getAllBossTemplates(): unknown[] {
    return Object.values(BOSS_TEMPLATES) as unknown[];
}

export function getBossTemplate(templateId: string): unknown | undefined {
