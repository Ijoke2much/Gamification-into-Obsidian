...
export class SkillBasedBattleEngine extends BossBattleEngine {
    ...
    static async processSkillGains(
        ...
        try {
            ...
            if (file instanceof TFile) {
                const content = await vault.read(file);
                const { data, content: markdownContent } = isMobile ? parseFrontmatterMobile(content) : matter(content);
                ...
                if (isMobile) {
                    ...
                    const updatedContent = yamlLines.join('\n') + '\n' + markdownContent;
    ...
    static async initializeSkillBasedBattle(
        ...
        const baseBattleState: BossBattleState = {
            ...
            playerStats: {
                ...
                specialDefense: playerBattleStats.wisdom || 10,
                buffs: [],
                debuffs: []
            },
            battleLog: [],
