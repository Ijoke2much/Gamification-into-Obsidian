...
export class TaskIntegrationService {
    ...
    private determinePriority(taskText: string): 'low' | 'medium' | 'high' {
        const text = taskText.toLowerCase();

        if (text.includes('urgent') || text.includes('critical') || text.includes('!!') || text.includes('asap')) {
            return 'high';
        }
    ...
    private calculateTaskDamage(taskType: 'checkbox' | 'bullet' | 'heading', priority: 'low' | 'medium' | 'high'): number {
