...
export class EnhancedBattleIntegration {
    ...
    private applyModifiersToStats(playerStats: PlayerStats, modifiers: FinalBattleModifiers): PlayerStats {
        return {
            ...playerStats,
            focus: (playerStats.focus || playerStats.mindfulness || 1) * (1 + modifiers.focusBonus),
            endurance: playerStats.endurance * (1 + modifiers.enduranceBonus),
            motivation: (playerStats.motivation || playerStats.willpower || 1) * modifiers.energyEfficiency
        };
    }
    ...
    private calculateBaseDamage(action: PlayerBattleAction, stats: PlayerStats): number {
        ...
        switch (action.type) {
            case 'focus_attack':
                baseDamage *= (1 + (stats.focus || stats.mindfulness || 1) / 100);
                break;
            case 'endurance_attack':
                baseDamage *= (1 + stats.endurance / 100);
                break;
            case 'motivation_attack':
                baseDamage *= (1 + (stats.motivation || stats.willpower || 1) / 100);
                break;
            case 'balanced_attack':
                baseDamage *= (1 + ((stats.focus || stats.mindfulness || 1) + stats.endurance + (stats.motivation || stats.willpower || 1)) / 300);
                break;
        }
